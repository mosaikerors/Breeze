...
class SignUp extends React.Component{
    constructor(props) {
        super(props);
        this.state = {username:'',password:'',confirmPass:'',phone:'',time: 30, btnDisable: false, btnContent:'发送邮件认证'};
                    ...
                    <Paper className={this.props.classes.paper}>
                        ...
                        <form onSubmit={event => {
                            ...
                            if (this.state.password === this.state.confirmPass) {
                                this.props.checkAccount(this.state.username, this.state.password, this.state.phone).then(()=>{sendCode();alert("注册成功！请前往邮箱认证！")}).catch(errorHandler);
                            } else {
                                alert('前后密码不一致');
                            }
                        }}>
...
function mapDispatchToProps(dispatch) {
    return {
        checkAccount: (username, password, phone) => {
            return User.signup(username, password, phone).then(res=>dispatch({type: "SIGN_UP"}));
        },
